...
3 In /docs, edit .md files to add documentation in markdown
4. Save files, run `sphinx-build -b html ./docs/ ./docs/_build/` (if that fails try `python -m sphinx.cmd.build ./docs/ ./docs/_build`) in root directory to create a preview of docs
5. Commit and push to repo when ready
